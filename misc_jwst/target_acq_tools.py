...
import misc_jwst.engdb, misc_jwst.oss_ta_sams
...
@functools.lru_cache
def get_visit_ta_image(visitid, verbose=True, kind='cal', inst='NIRCam', index=0, localpath=None, save_localpath=False, local_directory="./"):
    ...
    for filename in filenames:
        ...
        local_file_cache_path = localpath if localpath is not None else os.path.join(local_directory, filename)
...
########## MULTI INSTRUMENT ##########
# (Originally written for NIRCam, now generalized)

def main_ta_analysis(visitid, inst='NIRCam', verbose=True,  plot=True, output_plot_dir=".", return_wcs_offsets=False, return_pointing_offsets=False, **kwargs):
    ...
    # Preparation: Get metadata
    visitid = utils.get_visitid(visitid)
    inst = inst.upper()
    siaf = utils.get_siaf(inst)

    # Preparation: Get TA image(s)
    ...
    # Iterate to analyze and plot the TA image(s)
    for i_ta_image in range(n_ta_images):
        ...
        try:
            ...
            # TODO: trying to switch formats here just causes hassles and incompatibility... leave it for another time.
            #osslog = misc_jwst.engdb.get_oss_log_messages(start_time=hdul[0].header['VSTSTART'],
            #                                              end_time=hdul[0].header['VISITEND'])
            try:
            ...
            ### OSS SAM.
            # This is derived from, but not the same as, the centroid offset.
            # this may need some try/except error handling wrapper?
            try:
                #oss_sam = misc_jwst.engdb.extract_oss_TA_SAM(osslog,  visitid)
            ...
            ### WCS COORDINATES ###
            # Where was the target expected to be, at initial position post FGS ID->FG
            model = jwst.datamodels.open(hdul)
        ...
        except ImportError:  # I no longer understand why this try/except is here... (2025-11)
            oss_centroid_text = ""
            wcs_text = ""

        ### LOCAL CENTROIDS ###
        # We recompute locally on the ground a centroid using poppy.fwcentroid. This should be like what OSS did on board, in principle...
        #
        ...
        # (But this is not exactly like what happens onboard any more)
        nm = 6
        ...
        if plot:
            if inst=='MIRI':
                print('Need to update plotting code for subarray calc in full frame image')
            
            axes[i_ta_image].scatter(cen[1], cen[0], color='red', marker='+', s=50)
            axes[i_ta_image].text(cen[1], cen[0], '  stpsf', color='red', verticalalignment='center', clip_on=True)
        ...
        image_text = f"Pixel coordinates (0-based):         \n{aperture_text}\n{oss_centroid_text}\n stpsf measure_centroid: {cen[1]:.2f}, {cen[0]:.2f}\n{wcs_text}\n$\\Delta$pos ({deltapos_type}): {deltapos[0]:.2f}, {deltapos[1]:.2f}\n= {deltapos[0]*ta_aperture.XSciScale:.3f}, {deltapos[1]*ta_aperture.YSciScale:.3f} arcsec"
        ...
        if oss_cen_sci_pythonic is not None and (i_ta_image == show_oss_for_image):
            ...
            if plot:
                ...
                axes[show_oss_for_image].text(0.95, 0.63,
                ...
                if oss_sam is not None:
                    axes[show_oss_for_image].text(0.95, 0.75,
                                      f'TA SAM (V2, V3)=\n {oss_sam[0]:.3f}, {oss_sam[1]:.3f} arcsec',
                                      horizontalalignment='right', verticalalignment='bottom',
                                      transform=axes[show_oss_for_image].transAxes,
                                      color='yellow')
    ...
    if plot:
        ...
        fig.text(0.04, 0.005, f"Analysis on {now.isot[0:16]}, using file from MAST SDP {hdul[0].header['SDP_VER']}, pipeline {hdul[0].header['CAL_VER']}",
                 fontsize='x-small', color='0.5')

        plt.tight_layout()


        outname = os.path.join(output_plot_dir, f'{inst.lower()}_ta_analysis_{visitid}.pdf')
        plt.savefig(outname)
        print(f" => {outname}")
...
nrc_ta_analysis = main_ta_analysis  # alias for back compatibility
...
def nis_ta_analysis(visitid,  verbose=True, **kwargs):
    ...
    See main_ta_analysis for further docs """

    return main_ta_analysis(visitid, inst='NIRISS', verbose=verbose, **kwargs)
...
def miri_ta_analysis(visitid,  verbose=True, **kwargs):
    ...
    return main_ta_analysis(visitid, inst='MIRI', verbose=verbose, **kwargs)
...
def nirspec_wata_ta_analysis(visitid, verbose=True, show_centroids=True):
    ...
    for i_ta_image in range(n_ta_images):
        ...
        # offset WCS - oSS
        wcs_offset = [targ_coords_pix[i] - (oss_centroid[i] - 1) for i in range(2)]
       ...
       # Check subarray name, if necessary working around API changes in datamodels
        ...
        # close the box, by copying the first corner to the end
...
def auto_ta_results_analysis(visitid, verbose=True):
    ...
    if inst == 'NIRCAM':
        ...
        return miri_ta_analysis(visitid, verbose=verbose)
